...
Touch is **not** wired through `Engine::run()` automatically. Platform code should call `TouchManager::update`, then `getEvents`, then feed the current `Scene` via `processTouchEvents` (see `Engine.h` class comment and [Touch Input Architecture](/architecture/ARCH_TOUCH_INPUT)).
...
**Scene hooks** (see [API Core](/api/core/engine.md#scene)): `processTouchEvents` dispatches to UI (if enabled) then `onUnconsumedTouchEvent`.
...
| `bool init()` | Initializes adapter; call **`setCalibration` first** (see [Touch Input Architecture](/architecture/ARCH_TOUCH_INPUT)). |
...
Order after read: **map → vendor coords → mirror X → CAL offsets → clamp** (see [Touch Input Architecture](/architecture/ARCH_TOUCH_INPUT)).
