...
| `PIXELROOT32_TFT_ESPI_LINES_PER_BLOCK` | TFT_eSPI `sendBufferScaled`: RGB565 lines per DMA batch (when `PIXELROOT32_USE_TFT_ESPI_DRIVER`). Higher = fewer DMA rounds, more RAM for line buffers. | `60` (`PlatformDefaults.h`) |
| `PIXELROOT32_TFT_ESPI_LINES_PER_BLOCK_FALLBACK` | Line batch size retried if primary DMA line buffers cannot be allocated. | `30` (`PlatformDefaults.h`) |
