...
> `onEnter`/`onExit` edges, and the engine-owned event bus delivers what
> happens next. No other demo in this repository owns that path.
>
> It is honest about its size. Six systems are in play — 4bpp tilemaps,
> `StaticTilemapLayerCache`, dirty regions, a gravity-and-climbing controller,
> interaction triggers and the event bus — which is more than the one-idea
> rule in `CONTRIBUTING.md` asks for. Splitting the trigger-and-bus path into
> its own minimal `gameplay/interaction_triggers` demo is tracked as a
> follow-up; this migration moves the project as it stands rather than
...
- **State machine**: `IDLE`, `RUN`, `JUMP`, `CLIMBING` with sprite animation per state, dispatched through the engine's **`gameplay::StateMachine`** rather than a hand-written `switch`. The state graph is declared in a `static const` table on `PlayerActor`; `IDLE`/`RUN`/`JUMP` carry `onUpdate` callbacks that request the next transition. `CLIMBING`'s callback is deliberately null — that state is entered and left by ladder presence and vertical input, from outside the machine.
  ...
  Animation timing keeps an explicit accumulator reset from `onEnter`, rather than deriving frames from `getTimeInState()`. That is not an oversight: `StateMachine::update()` adds the frame delta *before* dispatching `onUpdate`, and `requestState()` then resets time-in-state to `0`, so a transition made from inside `onUpdate` would lose one frame per transition. See the `@warning` on `update()` in [`gameplay/StateMachine.h`](https://github.com/PixelRoot32-Game-Engine/PixelRoot32-Game-Engine/blob/main/include/gameplay/StateMachine.h).
...
**Source code:** https://github.com/PixelRoot32-Game-Engine/PixelRoot32-Demo-Projects/tree/main/gameplay/metroidvania
